...
async function buildTeamPlayers(draftId: number, userId: number): Promise<TeamPlayer[]> {
  const picks = await db
    .select({
      ...
      ppg: sql<number>`coalesce(nullif(avg(${playerSeasonStats.ppg}), 'NaN'), 0)`,
      rpg: sql<number>`coalesce(nullif(avg(${playerSeasonStats.rpg}), 'NaN'), 0)`,
      apg: sql<number>`coalesce(nullif(avg(${playerSeasonStats.apg}), 'NaN'), 0)`,
      spg: sql<number>`coalesce(nullif(avg(${playerSeasonStats.spg}), 'NaN'), 0)`,
      bpg: sql<number>`coalesce(nullif(avg(${playerSeasonStats.bpg}), 'NaN'), 0)`,
      fgPct: sql<number>`coalesce(nullif(avg(${playerSeasonStats.fgPct}), 'NaN'), 0)`,
      ftPct: sql<number>`coalesce(nullif(avg(${playerSeasonStats.ftPct}), 'NaN'), 0)`,
      threePct: sql<number>`coalesce(nullif(avg(${playerSeasonStats.threePct}), 'NaN'), 0)`,
      minutesPg: sql<number>`coalesce(nullif(avg(${playerSeasonStats.minutesPg}), 'NaN'), 0)`,
    })
    .from(draftPicks)
